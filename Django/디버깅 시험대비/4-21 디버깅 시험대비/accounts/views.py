...
def delete(request, user_pk):
    user = get_user_model()
    if request.user == user.objects.get(pk=user_pk):
        request.user.delete()
    return redirect('movies:index')
...
def profile(request, user_pk):
    user = get_user_model()
    detail = user.objects.get(pk=user_pk)
    movies = Movie.objects.filter(user_id=user_pk)
    likes_movies = Movie.objects.filter(like_user=user_pk)
